...
struct WAVHeaderSpeaker {
  ...
  uint16_t audioFormat; uint16_t numChannels; uint32_t sampleRate; uint32_t byteRate;
...
void setupI2SSpeaker(uint32_t sampleRate) {
  // Uninstall I2S_PORT (microphone) ก่อน
  i2s_driver_uninstall(I2S_PORT);
  delay(50);

  // ตั้งค่า I2S สำหรับ Speaker (DAC)
...
void playWav(const char* filename) {
  ...
  unsigned long lastYield = millis();  // ← เพิ่มบรรทัดนี้
  ...
  while (file.available()) {
    ...
    // ← เพิ่มส่วนนี้: Reset Watchdog ทุก 100ms
    if (millis() - lastYield > 100) {
      yield();
      delay(1);  // ให้ Task Scheduler ทำงาน
      lastYield = millis();
    }
  ...
  // ลบไฟล์ต้นฉบับ (recording)
  originalFile.replace("_response.wav", ".wav");
  if (SPIFFS.exists(originalFile.c_str())) {
    SPIFFS.remove(originalFile.c_str());
    Serial.printf("🗑️ Deleted: %s\n", originalFile.c_str());
    fileCounter--; // ลดจำนวนไฟล์
  }
...
void loop() {
  ...
  if (!isRecording && digitalRead(SWITCH)) {
    if (detectWakeWord()) {
      isRecording = true;
